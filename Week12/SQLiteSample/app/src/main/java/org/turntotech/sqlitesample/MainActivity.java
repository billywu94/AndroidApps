...
public class MainActivity extends Activity {
	...
	public void sortClicked(View view){
		Cursor cursor = db.rawQuery("SELECT stud_name FROM Student ORDER BY stud_name", null);
		ArrayList<String> sortedNames = new ArrayList<>();
		if(cursor.moveToFirst()) {
			do {
				String name = cursor.getString(0);
				System.out.println("sorted: " + name);
				sortedNames.add(name); //store sorted order into ArrayList

			} while (cursor.moveToNext());
			cursor.close();
			data.clear(); //remove items from list
			for(int i = 0; i < sortedNames.size(); ++i){
				HashMap<String, String> sortMap = new HashMap<String, String>(); //create new HashMap
				sortMap.put("stud_name", sortedNames.get(i)); //add sorted names from ArrayList to HashMap
				data.add(sortMap); //add HashMap to data ArrayList
			}
			adapter.notifyDataSetChanged();
		}
	}
